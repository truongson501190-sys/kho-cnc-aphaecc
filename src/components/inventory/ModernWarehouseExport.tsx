...
import { Plus, Trash2, PackageOpen, Printer, Save, AlertTriangle } from 'lucide-react';
...
interface ModernWarehouseExportProps {
  onSubmit: (transaction: Omit<WarehouseTransaction, 'id' | 'createdAt'>) => void;
}

export function ModernWarehouseExport({ onSubmit }: ModernWarehouseExportProps) {
  const { currentUser } = useAuth();
  
  const [formData, setFormData] = useState({
    referenceNumber: `XK${new Date().getFullYear()}${String(Date.now()).slice(-6)}`,
    transactionDate: new Date().toISOString().split('T')[0],
    fromLocation: '',
    recipient: '',
    reason: 'Xuất hàng cho sản xuất',
    notes: '',
    ...
    try {
      ...
      if (savedItems) {
        ...
        const defaultItems: InventoryItem[] = [
          {
            id: '1', name: 'Thép tấm A36', code: 'STEEL001', category: 'Vật liệu',
            unit: 'kg', currentStock: 1000, minStock: 100, maxStock: 2000,
            location: 'Kho A', price: 25000, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
          },
          {
            id: '2', name: 'Dầu cắt gọt Castrol', code: 'OIL001', category: 'Dầu mỡ',
            unit: 'lít', currentStock: 500, minStock: 50, maxStock: 1000,
            location: 'Kho B', price: 45000, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
          }
        ];
  ...
  const addExportItem = () => {
    setExportItems(prev => [...prev, { itemId: '', itemName: '', quantity: 0, unit: '', availableStock: 0, price: 0, totalValue: 0 }]);
  };
  ...
  const updateExportItem = (index: number, field: keyof ExportItem, value: string | number) => {
    setExportItems(prev => prev.map((item, i) => {
      if (i === index) {
  ...
  const getTotalValue = () => exportItems.reduce((sum, item) => sum + item.totalValue, 0);
  const hasStockIssues = () => exportItems.some(item => item.quantity > item.availableStock && item.itemId);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    
    try {
      if (!formData.fromLocation || !formData.recipient) {
        toast.error('Vui lòng điền đầy đủ thông tin bắt buộc');
        return;
      }

      const validItems = exportItems.filter(item => item.itemId && item.quantity > 0);
      ...
      validItems.forEach(item => {
        const transaction: Omit<WarehouseTransaction, 'id' | 'createdAt'> = {
          ...
          reason: `${formData.reason} - Người nhận: ${formData.recipient}`,
      ...
      setFormData({
        referenceNumber: `XK${new Date().getFullYear()}${String(Date.now()).slice(-6)}`,
        transactionDate: new Date().toISOString().split('T')[0],
        fromLocation: '', recipient: '', reason: 'Xuất hàng cho sản xuất', notes: '', status: 'draft',
      });
      setExportItems([{ itemId: '', itemName: '', quantity: 0, unit: '', availableStock: 0, price: 0, totalValue: 0 }]);
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white">
      ...
      <div className="text-center border-b-2 border-gray-300 pb-4 mb-6 print:mb-4">
        ...
        <div className="mt-4">
          <h2 className="text-xl font-bold text-red-600 uppercase">PHIẾU XUẤT KHO</h2>
          <p className="text-sm text-gray-500">Warehouse Export Receipt</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-gray-300 p-4 rounded">
          ...
          <div>
            <Label className="font-semibold">Kho xuất / Warehouse *</Label>
        ...
        <div className="border border-gray-300 rounded">
          <div className="bg-gray-100 p-3 border-b border-gray-300">
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-gray-700">DANH SÁCH HÀNG HÓA / ITEMS LIST</h3>
            ...
            <table className="w-full">
              ...
              <tbody>
                {exportItems.map((item, index) => (
                  <tr key={index} className="border-b border-gray-200">
                    ...
                    <td className="p-2">
                      <Select 
                        value={item.itemId} 
                        onValueChange={(value) => updateExportItem(index, 'itemId', value)}
                      >
                    ...
                    <td className="p-2">
                      <Input
                        type="number"
                        value={item.quantity}
                        onChange={(e) => updateExportItem(index, 'quantity', parseFloat(e.target.value) || 0)}
                    ...
                    <td className="p-2">
                      <Input
                        type="number"
                        value={item.price}
                        onChange={(e) => updateExportItem(index, 'price', parseFloat(e.target.value) || 0)}
                ...
                <tr className="bg-gray-50 border-t-2 border-gray-400">
                  ...
                  <td className="p-3 text-right font-bold text-lg font-mono text-red-600">
        ...
        <div className="border border-gray-300 p-4 rounded">
          ...
          <Textarea
            ...
            placeholder="Ghi chú thêm về phiếu xuất kho..."
            rows={3}
            className="mt-2"
          />
        </div>
        ...
        <div className="grid grid-cols-3 gap-8 text-center mt-8 print:mt-12">
          ...
          <div>
            ...
            <p className="border-t border-gray-400 pt-2">{formData.recipient}</p>
          </div>
        </div>
